  ...
  },
{
  ...
  {
    id:5, label:"Day 5", title:"Merging Data",
    subtitle:"Merge vs relationship, the postcode merge, and building your own reference table", phase:"powerbi",
    recap:"Last time was a big one — dashboards explained, your full 4-table star schema connected, and your first 5 visuals built. Today is focused on one specific skill: merging. It's different to a relationship and today you'll feel that difference for yourself.",
    sections:[
      {type:"intro", content:"Merging is one of the most useful things in a consultant's toolkit. Today you'll do the postcode merge, then build your own therapist reference table from scratch and merge that in too — including using Power Query's Index feature to give each therapist a proper ID. By the end of today you'll have Region on your Patients table and TherapistRole on your Sessions table."},
      {type:"lesson", title:"Merge vs Relationship — What's the Difference?", content:"A relationship (what you built on Day 4) is a live link between two tables. The tables stay separate, but Power BI knows how to connect them when you build a visual. Nothing gets copied — it all happens on the fly.\n\nA merge is different. A merge physically pulls columns from one table into another, creating a combined result. It's the Power Query equivalent of a VLOOKUP — you're looking up a value in one table and bringing it across into another.\n\nYou'd use a relationship when you want two tables to stay separate but linked (like Patients and Sessions). You'd use a merge when you specifically want to enrich one table with extra columns from another — which is exactly what you're about to do with postcodes.",
        video:{label:"How To Easily Merge Tables With Power Query: VLOOKUP Alternative (Excel Campus)", url:"https://www.youtube.com/watch?v=73T7isNVH_w", note:"Watch this before starting — explains merging in the exact terms you'll recognise from VLOOKUP."}},
      {type:"lesson", title:"Watch This Too — Join Types Explained", content:"This one explains the different join types visually — Left Outer, Inner, Full Outer and more. Worth watching before you start so you understand what each one keeps and drops.", video:{label:"Power Query Merge Join Types Explained", url:"https://www.youtube.com/watch?v=vFL4Z4nsx6c", note:"Really clear visual walkthrough of every join type."}},
      {type:"lesson", title:"Which Join Type Should You Use?", content:"In almost every real-world merge you'll do as a consultant, the answer is Left Outer. Here's why:\n\nLeft Outer — keeps every row from your main table (the left one) and brings in matching data from the lookup table where it exists. If there's no match, you get a blank — but crucially, you don't lose any rows.\n\nInner — only keeps rows where a match exists on both sides. Anything without a match gets silently dropped. This is dangerous if you don't know your data perfectly.\n\nRight Outer / Full Outer — less common. Right Outer keeps everything from the lookup table. Full Outer keeps everything from both sides.\n\nThe golden rule: default to Left Outer so you never accidentally lose data. If you want to experiment with Inner or Full Outer to see what changes, go for it — then delete that Applied Step and redo it as Left Outer. Power Query makes this completely safe to play with.",
        callout:"Experiment freely — try a different join type, see what changes, then delete the Applied Step and go back to Left Outer. Nothing is permanent in Power Query."},
      {type:"lesson", title:"The Postcode Merge", content:"Your Patients table has a Postcode column — things like 'SW16 7PQ'. On its own that's not useful for reporting. But you've got a PostcodePrefixes.xlsx file that maps postcode prefixes (like 'SW16') to a Region.\n\nThe problem: Patients has the full postcode ('SW16 7PQ') but PostcodePrefixes only has the prefix ('SW16'). They won't match directly — you need to extract just the prefix first.",
        tip:"This is a genuinely realistic scenario. Real-world data almost never lines up perfectly for a merge first try — you'll usually need to clean or extract something first."},
      {type:"tasks", title:"Postcode Merge — Step by Step", file:"PostcodePrefixes.xlsx", tasks:[
        ...
        {id:"pm2", text:"In the Patients query, add a Custom Column called PostcodePrefix. Use: Text.BeforeDelimiter([Postcode], \" \") — this grabs everything before the space, so 'SW16 7PQ' becomes 'SW16'."},
        {id:"pm3", text:"Check your new PostcodePrefix column against PostcodePrefixes — do the values now match?"},
        {id:"pm4", text:"On the Patients query, go to Home → Merge Queries. Select PostcodePrefixes, match on PostcodePrefix in both tables."},
        {id:"pm5", text:"Use Left Outer join — keeps every patient even if a postcode prefix doesn't match."},
        {id:"pm6", text:"Expand and select just Region. Untick 'Use original column name as prefix' so it's just called Region."},
        {id:"pm7", text:"Close & Apply. You now have a Region column on Patients sourced entirely from a separate file."},
        {id:"pm8", text:"Bonus: try switching to an Inner join — how many rows do you lose? Then delete that Applied Step and go back to Left Outer."},
      ]},
      {type:"lesson", title:"Second Merge — Build Your Own Reference Table", content:"For this merge you're going to create the reference table yourself in Excel, then load and merge it. This is a very common real-world scenario — the data you need doesn't exist anywhere, so you build it.\n\nYou've got four therapists in your Sessions table: Dr. Lena Marsh, Solomon Attah, Berivan Sahin, Sarah Obi. You're going to create a Therapists reference table with their names, roles, and — using Power Query's Index feature — a proper TherapistID.\n\nFor TherapistRole, make up something realistic for a talking therapies service. There are no wrong answers — Senior PWP, Trainee PWP, Clinical Psychologist, whatever feels right. You're the consultant building the table."},
      {type:"tasks", title:"Build the Therapist Table & Merge", tasks:[
        {id:"t1", text:"Open a new Excel file. Create two columns: TherapistName and TherapistRole. Add one row per therapist — Dr. Lena Marsh, Solomon Attah, Berivan Sahin, Sarah Obi — and assign each a role of your choosing. Save as Therapists.xlsx."},
        {id:"t2", text:"Load Therapists.xlsx into Power BI via Get Data → Transform Data."},
        {id:"t3", text:"In the Therapists query, go to Add Column → Index Column → From 1. This adds a sequential number starting at 1."},
        {id:"t4", text:"Now create a proper ID. Add a Custom Column with: \"T-\" & Text.PadStart(Text.From([Index]), 3, \"0\"). This turns 1 into T-001, 2 into T-002 and so on."},
        {id:"t5", text:"Rename that new column TherapistID. Delete the original Index column — you just needed it to generate the ID."},
        {id:"t6", text:"Close & Apply. Then go back into Transform Data on the Sessions query."},
        {id:"t7", text:"On Sessions, go to Home → Merge Queries. Select Therapists, match on TherapistName. Use Left Outer."},
        {id:"t8", text:"Expand and bring in TherapistID and TherapistRole. Close & Apply."},
        {id:"t9", text:"Check for blanks in the TherapistID column — if 'Soloman Attah' (the typo) wasn't cleaned earlier, those rows won't have matched. This is the merge silently missing a near-match."},
      ]},
      {type:"resources", title:"Watch These", links:[
        {label:"How To Easily Merge Tables With Power Query: VLOOKUP Alternative (Excel Campus)", url:"https://www.youtube.com/watch?v=73T7isNVH_w", note:"The main video for today"},
        {label:"Power Query Merge Join Types Explained", url:"https://www.youtube.com/watch?v=vFL4Z4nsx6c", note:"Clear visual explanation of Left Outer vs Inner vs Full Outer"},
      ]},
      {type:"quiz", questions:[
        ...
        {q:"Patients has Postcode = 'SW16 7PQ'. PostcodePrefixes has PostcodePrefix = 'SW16'. Why can't you merge these directly?", options:["They're different data types","The values don't match exactly — one has extra characters the other doesn't","Power Query doesn't support postcodes","You need a relationship instead"], answer:1, explanation:"A merge needs matching values on both sides. 'SW16 7PQ' and 'SW16' are not identical, so you first need to extract the prefix before the merge will work."},
        {q:"Which join type should you use so no patients are accidentally dropped from the postcode merge?", options:["Inner join","Left Outer join — keeps every row from Patients regardless of a match","Right Outer join","Full Outer join"], answer:1, explanation:"A Left Outer join keeps every row from your primary table (Patients) and brings in matching data where it exists. An Inner join would silently drop any patient whose postcode prefix didn't match."},
        {q:"You try an Inner join on the postcode merge and the row count drops from 30 to 27. What happened?", options:["Power BI deleted duplicates","3 patients had postcode prefixes with no match in PostcodePrefixes and were silently dropped","The merge failed","The Index column caused a conflict"], answer:1, explanation:"Inner join only keeps rows that match on both sides. The 3 patients with unrecognised prefixes had no match and were dropped entirely. This is why Left Outer is the safe default."},
        {q:"What does the Index Column feature in Power Query do?", options:["It creates a relationship to another table","It adds a sequential number to each row — useful as the basis for a unique ID","It sorts the table alphabetically","It removes duplicates"], answer:1, explanation:"Index Column adds a sequential number to each row. On its own it's just a number — but combined with a Custom Column formula you can turn it into a formatted ID like T-001."},
      ]},
      {type:"hardquiz", questions:[
        {q:"After your postcode merge, three patients show a blank Region. What does this most likely mean?", options:["The merge failed completely","Their postcode prefix doesn't exist in PostcodePrefixes.xlsx — the lookup table doesn't cover every prefix","Power BI has a bug","You used the wrong join type"], answer:1, explanation:"A blank after a Left Outer join means no match was found — the postcode prefix simply isn't in your reference table. This is common in real consultancy work and you'd flag it to a client."},
        {q:"Your therapist merge silently misses rows where the name was typed 'Soloman Attah'. What's the lesson?", options:["Merges are unreliable","A merge is only as accurate as the exactness of the values you're matching on — inconsistent text breaks it silently with no error","You need a relationship instead","TherapistName should be a number"], answer:1, explanation:"A merge won't throw an error for a near-miss — it just won't match, and the row quietly gets a blank. This is why cleaning text columns early matters."},
        {q:"Why prefer a relationship for Patients → Sessions but a merge for bringing Region from postcodes?", options:["Merges are always better","Relationships keep large fact tables efficient and dynamic; merges suit smaller reference enrichments where you want extra descriptive columns added permanently","It doesn't matter","Merges are required for tables over 20 rows"], answer:1, explanation:"Relationships are efficient for connecting tables that stay large and central to your model. Merges suit smaller descriptive enrichments like adding a Region label permanently."},
      ]},
    ],
  },
  {
    id:6, label:"Day 6", title:"DAX — Calculated Columns & Measures",
    subtitle:"Your first DAX formulas — columns that live in the table and measures that think for themselves", phase:"powerbi",
    recap:"Yesterday you merged data — postcodes into Region, and built your own Therapists reference table with proper IDs. Today you move into DAX, the formula language that makes Power BI genuinely powerful.",
    sections:[
      {type:"intro", content:"DAX stands for Data Analysis Expressions. It's Power BI's formula language — what you use to add new calculated columns to your tables and write calculations that respond dynamically to filters. It looks a bit like Excel formulas but it thinks differently. Today you'll learn the difference between columns and measures, use the cheat sheet as your reference, and use ChatGPT as your assistant to help with syntax when you need it."},
      {type:"lesson", title:"What Is DAX?", content:"DAX is a language designed specifically for working with relational data tables. You use it in two ways:\n\nCalculated Columns — you add a new column to a table and write a formula that calculates row by row. It gets stored in the table permanently. It doesn't change when you apply filters.\n\nMeasures — you write a formula that calculates dynamically based on whatever's currently selected on the report. It responds to slicers, filters, visuals — everything. Measures don't live in a table, they float and recalculate constantly.\n\nThink of it this way: a Calculated Column answers 'how old is this patient in months?' for every row, once. A Measure answers 'what is the average age in months of all patients currently filtered?' and recalculates the moment anything on the report changes.",
        video:{label:"DAX Measures vs Calculated Columns (Guy in a Cube)", url:"https://www.youtube.com/watch?v=53RFhSzBBi8", note:"Watch this first — the distinction between columns and measures is the most important thing to understand in DAX."}},
      {type:"lesson", title:"Your DAX Cheat Sheet", content:"DataCamp have put together a solid DAX cheat sheet. Keep it open today — you don't need to memorise any of it, you just need to know it exists and roughly what each function does. Functions are grouped by category: aggregation, logical, date/time, text.",
        video:{label:"DAX Cheat Sheet — DataCamp", url:"https://www.datacamp.com/cheat-sheet/dax-cheat-sheet", note:"Bookmark this. Refer back whenever you need to find the right function."}},
      {type:"lesson", title:"Use AI — Seriously", content:"DAX is one of those areas where AI is genuinely useful as a learning tool. You know what question you're trying to answer — you might not know the exact syntax. That's fine.\n\nDescribe what you want in plain English and ask ChatGPT to write the DAX. Then paste it into Power BI, check if the output looks right, and adjust if needed.\n\nFor example: 'I have a Patients table with a DateOfBirth column. Write me a DAX calculated column that shows how old each patient is in months.'\n\nChatGPT will give you something. Try it. The skill is knowing what to ask for and being able to judge whether the result makes sense — not memorising syntax.",
        callout:"The skill isn't memorising DAX. It's knowing what you want to calculate, describing it clearly, and knowing when the output looks right or wrong."},
      {type:"lesson", title:"Where to Write DAX", content:"Calculated Columns: go to Table view → select the table you want to add the column to → click 'New Column' in the ribbon. The formula bar appears at the top.\n\nMeasures: right-click any table in the Fields pane → New Measure. Or go to Home → New Measure. Measures can technically live in any table — put them in the table they're most related to.\n\nSyntax: always reference columns as TableName[ColumnName]. Reference other measures in square brackets like [MeasureName]."},
      {type:"tasks", title:"Calculated Columns — Try These", tasks:[
        {id:"dc1", text:"MonthsOld — add a calculated column to Patients showing how old each patient is in months. Use DateOfBirth and TODAY(). Try writing it yourself first, then check the cheat sheet or ask ChatGPT if you get stuck."},
        {id:"dc2", text:"DaysOld — same idea but in days. How many days old is each patient?"},
        {id:"dc3", text:"DaysTilRetirement — assuming retirement age is 67, how many days until each patient retires? You'll need to calculate their retirement date from DateOfBirth and then find the difference from today."},
        {id:"dc4", text:"Try one more calculated column of your own — something that feels genuinely useful for a wellbeing service report. What else could you calculate from the columns you have?"},
      ]},
      {type:"lesson", title:"Your First Measures", content:"Here are a few starter measures. Write them, put each one on a card visual, and check the result looks right.",
        daxExamples:[
          {name:"Total Patients", formula:"Total Patients = COUNTROWS(Patients)", note:"Counts every row in the Patients table. The most basic measure — great on a card visual."},
          {name:"Oldest Patient (by DOB)", formula:"Oldest Patient DOB = MIN(Patients[DateOfBirth])", note:"MIN on a date column returns the earliest date of birth — meaning the oldest person. Counterintuitive at first: 1945 is earlier (smaller) than 1990."},
          {name:"Youngest Patient (by DOB)", formula:"Youngest Patient DOB = MAX(Patients[DateOfBirth])", note:"MAX returns the most recent date of birth — the youngest patient. Does the date that shows make sense?"},
        ],
      },
      {type:"tasks", title:"Measures to Build", tasks:[
        {id:"me1", text:"Write Total Patients and put it on a card. Does the number match the row count you'd expect?"},
        {id:"me2", text:"Write Oldest Patient DOB and Youngest Patient DOB. Put them on cards side by side. Do the dates look right for your dataset?"},
        {id:"me3", text:"Use the cheat sheet or ChatGPT to write a measure that calculates the average age across all patients. Format it to 1 decimal place."},
        {id:"me4", text:"Try writing a measure that counts only patients referred for Low Mood. Hint: you'll need CALCULATE. Describe what you want to ChatGPT and see what it gives you."},
        {id:"me5", text:"Write at least two more measures of your own choosing. Start from the questions — what do you actually want to know about this dataset?"},
      ]},
      {type:"resources", title:"Watch & Read", links:[
        {label:"DAX Measures vs Calculated Columns (Guy in a Cube)", url:"https://www.youtube.com/watch?v=53RFhSzBBi8", note:"Watch first"},
        {label:"DAX Cheat Sheet — DataCamp", url:"https://www.datacamp.com/cheat-sheet/dax-cheat-sheet", note:"Keep this open all day"},
        {label:"DAX Guide — dax.guide", url:"https://dax.guide", note:"More detailed function reference if you want to go deeper"},
      ]},
      {type:"quiz", questions:[
        {q:"What's the core difference between a DAX Calculated Column and a Measure?", options:["No real difference","A Calculated Column computes row by row and lives in the table permanently; a Measure computes dynamically based on whatever filters are active","Measures are always slower","Calculated Columns only work with text"], answer:1, explanation:"Calculated Columns are static — they calculate once and get stored. Measures are dynamic — they recalculate every time the filter context changes."},
        {q:"You write: Oldest Patient DOB = MIN(Patients[DateOfBirth]). Why does MIN return the oldest patient?", options:["MIN always returns the oldest value","An earlier date of birth means an older person — 1945 is earlier than 1990, so MIN finds the most elderly patient","It's a quirk with no logic","MIN doesn't work on dates"], answer:1, explanation:"Older people were born earlier in time, so their date of birth is a smaller value. MIN returns the smallest (earliest) date — which corresponds to the oldest person."},
        {q:"Where do you write a DAX Calculated Column in Power BI?", options:["In the Power Query formula bar","In Table view — select the table, click New Column in the ribbon","In Report view by right-clicking a visual","In the Relationships pane"], answer:1, explanation:"Calculated Columns are written in Table view. Select the table, click New Column, and write your DAX in the formula bar at the top."},
      ]},
      {type:"hardquiz", questions:[
        {q:"You ask ChatGPT for a DAX measure for average age and it gives you: Avg Age = AVERAGE(Patients[Age]). It shows 35.2. How do you know if this is right?", options:["You trust ChatGPT","Cross-check it: go to Table view, look at the Age column, and judge whether 35.2 feels like a reasonable average for your patients","Run it in Excel","It must be right if Power BI accepted it"], answer:1, explanation:"Always sanity check outputs. ChatGPT can write correct syntax but it can't know if the result makes sense for your specific data. You have to look at the actual values and use your own judgement."},
        {q:"Your DaysTilRetirement calculated column shows a negative number for one patient. What does this tell you?", options:["The formula has a bug","That patient is already past retirement age — a negative number means their retirement date has already passed","The DateOfBirth has an error","DAX can't handle negative numbers"], answer:1, explanation:"A negative result means the retirement date has already passed — the patient is over 67. This is genuinely useful information and a good example of how a calculated column can surface things worth knowing."},
        {q:"You write: Low Mood Count = CALCULATE(COUNTROWS(Patients), Patients[PresentingProblem] = \"Low Mood\"). You add a Nationality slicer and select Turkish. What does the measure show?", options:["All Low Mood patients regardless of nationality","Only Turkish patients with Low Mood — the slicer and CALCULATE's filter both apply","An error","Zero — CALCULATE ignores external filters"], answer:1, explanation:"CALCULATE adds to the existing filter context — it doesn't replace it. The slicer applies first (Turkish only), then CALCULATE filters within that for Low Mood. The result is Turkish patients with Low Mood."},
      ]},
    ],
  },
